...
namespace VC.Public.Controllers
{
    public class CheckoutController : CheckoutControllerBase
    {
        ...
        private string ResolveStateOrCounty(ICollection<Country> countries, AddressDynamic address)
        {
            ...
            var stateOrCounty = address.IdState.HasValue
                ? target.States.Single(x => x.Id == address.IdState.Value).StateCode
                : address.County;

            return stateOrCounty;
        }

        private List<KeyValuePair<string, string>> PopulateBillingAddressDetails(OrderPaymentMethodDynamic paymentMethod,
            ICollection<Country> countries, string email)
        {
            ...
            return new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string, string>(string.Empty,
                    _appInfrastructure.CreditCardTypes.Single(z => z.Key == (int) paymentMethod.Data.CardType).Text),
                ...
                new KeyValuePair<string, string>(string.Empty,
                    $"{billingAddress.Data.City}, {ResolveStateOrCounty(countries, billingAddress)} {billingAddress.Data.Zip}"),
                new KeyValuePair<string, string>("Phone", string.Format("{0:(###) ###-#### x#####}", billingAddress.Data.Phone)),
                new KeyValuePair<string, string>("Email", email),
            };
        }

        private List<KeyValuePair<string, string>> PopulateShippingAddressDetails(AddressDynamic shippingAddress,
            ICollection<Country> countries)
        {
            return new List<KeyValuePair<string, string>>()
            {
                ...
                new KeyValuePair<string, string>(string.Empty,
                    $"{shippingAddress.Data.City}, {ResolveStateOrCounty(countries, shippingAddress)} {shippingAddress.Data.Zip}"),
