...
namespace VitalChoice.Business.Services.Impl.Content
{
    public class FAQService : IFAQService
    {
        ...
        public async Task<PagedList<FAQ>> GetFAQsAsync(string name = null, int? categoryId = null, int page = 1, int take = BaseAppConstants.DEFAULT_LIST_TAKE_COUNT)
        {
            ...
            if(categoryId.HasValue)
            {
                ...
                {
                    ids = (await faqToContentCategoryRepository.Query(p => p.ContentCategoryId == categoryId).SelectAsync(false)).Select(p => p.FAQId).ToList();
