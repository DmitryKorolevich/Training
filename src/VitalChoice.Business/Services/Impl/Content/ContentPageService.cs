...
namespace VitalChoice.Business.Services.Impl.Content
{
    public class ContentPageService : IContentPageService
    {
        ...
        public async Task<PagedList<ContentPage>> GetContentPagesAsync(string name = null, int? categoryId = null, int page = 1, int take = BaseAppConstants.DEFAULT_LIST_TAKE_COUNT)
        {
            ...
            if(categoryId.HasValue)
            {
                ...
                {
                    ids = (await contentPageToContentCategoryRepository.Query(p => p.ContentCategoryId == categoryId).SelectAsync(false)).Select(p => p.ContentPageId).ToList();
