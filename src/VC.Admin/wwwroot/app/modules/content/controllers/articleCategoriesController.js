angular.module('app.modules.content.controllers.articleCategoriesController', [])
.controller('articleCategoriesController', ['$scope', '$state', '$stateParams', 'contentService', 'toaster', 'modalUtil', 'confirmUtil', function ($scope, $state, $stateParams, contentService, toaster, modalUtil, confirmUtil) {
    ...
    $scope.open = function (id) {
        $state.go('index.oneCol.articleCategoryDetail', { id: id });
    };

    $scope.add = function(id)
    {
        $state.go('index.oneCol.addNewArticleCategory', { categoryid: id });
    }
    ...
    $scope.cancel = function () {
        if ($scope.mode == 'list') {
            $state.go('index.oneCol.manageArticles');
        }
        if ($scope.mode == 'edit') {
            if ($scope.id) {
                $state.go('index.oneCol.articleDetail', { id: $scope.id });
            }
            else {
