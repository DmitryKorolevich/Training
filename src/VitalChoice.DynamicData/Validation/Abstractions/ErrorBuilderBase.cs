...
namespace VitalChoice.DynamicData.Validation.Abstractions
{
    public abstract class ErrorBuilderBase<TProperty> : IDataContainer<TProperty>, IErrorResult
    {
        ...
        internal static string GetModelName(string dynamicName, Type modelType)
        {
            if (modelType != null)
            {
                var typeCache = DynamicTypeCache.GetTypeCache(modelType);
                return typeCache.Properties.FirstOrDefault(m => (m.Value.Map?.Name ?? m.Key) == dynamicName).Key;
            }
            return dynamicName;
        }
        ...
        public List<MessageInfo> Build()
        {
            if (!string.IsNullOrEmpty(CollectionName))
            {
                ...
                {
                    Field = PropertyName.FormatCollectionError(CollectionName, i),
                    Message = ErrorText
                }).ToList();
            }
