...
namespace VitalChoice.Ecommerce.Domain.Helpers.Async
{
    internal sealed class Deque<T> : IList<T>, System.Collections.IList
    {
        ...
        /// <summary>
        /// The _offset into <see cref="_buffer"/> where the view begins.
        /// </summary>
        private int _offset;
        ...
        /// <exception cref="ArgumentException">The range [_offset, _offset + count) is not within the range [0, sourceLength).</exception>
        private static void CheckRangeArguments(int sourceLength, int offset, int count)
        {
            if (offset < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(offset), "Invalid _offset " + offset);
            }
            ...
            {
                throw new ArgumentException("Invalid _offset (" + offset + ") or count + (" + count + ") for source length " + sourceLength);
            }
        }
        ...
        /// <summary>
        /// Gets a value indicating whether the _buffer is "split" (meaning the beginning of the view is at a later index in <see cref="_buffer"/> than the end).
        /// </summary>
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                if (IsSplit)
                {
                    // The existing _buffer is split, so we have to copy it in parts
        ...
        /// <summary>
        /// Applies the _offset to <paramref name="index"/>, resulting in a _buffer index.
        /// </summary>
        /// <param name="index">The deque index.</param>
        /// <returns>The _buffer index.</returns>
