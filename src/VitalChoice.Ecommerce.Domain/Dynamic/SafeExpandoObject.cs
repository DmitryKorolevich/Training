...
{
    public sealed class SafeDynamicObject : DynamicObject
    {
        private IDictionary<string, object> _dictObj;

        public SafeDynamicObject(ExpandoObject obj)
        {
            Initialize(obj);
        }

        public void Initialize(ExpandoObject obj)
        {
            _dictObj = obj;
        }
        ...
        {
            _dictObj.TryGetValue(binder.Name, out result);
            return true;
        }
    ...
    //{
    //    private IDictionary<string, object> _dictObj;
    //    private IEnumerable<KeyValuePair<string, object>> _enumerable;
    ...
    //                    result.Expression
    //                    ),
    //                result.Restrictions.Merge(fallback.Restrictions), Value
    //                );
