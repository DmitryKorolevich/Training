...
SELECT 
	...
	CAST(shipOptions.PreferredShipMethod as int) As PreferredShipMethod
	FROM Orders AS o
	...
	LEFT JOIN (SELECT [IdOrderAddress], [FirstName], [LastName], [Company], [PreferredShipMethod]
	...
	PIVOT(
	MIN([Value]) FOR [Name] in ([FirstName], [LastName], [City], [Company], [PreferredShipMethod])
