...
namespace Authorize.Net.AIM.Requests
{
    ...
    {
        private readonly StringBuilder _post;
        ...
        protected GatewayRequest()
        {
            ...
            LoadCoreValues();
            ApiAction = RequestAction.AuthorizeAndCapture;
        }

        //public Dictionary<string,string> Post { get; set; }
        public NameValueCollection Post { get; set; }
        public RequestAction ApiAction { get; private set; }
        ...
        public string ToPostString()
        {
            ...
            {
                var key = Post.GetKey(i);
                var values = Post.GetValues(i);
        ...
        /// </summary>
        public IGatewayRequest AddCustomer(string ID, string email, string first, string last, string address, string city, string state,
            string zip)
        {
        ...
        /// <summary>
        ///     This method adds the required values for Fraud Detection Suite. Your merchant must sign up for this service with
        ///     Authorize.Net
        /// </summary>
        ...
        {
            Freight = $"{name}<|>{description}<|>{amount.ToString(CultureInfo.InvariantCulture)}";
            return this;
        }
        ...
        {
            Freight = amount.ToString(CultureInfo.InvariantCulture);
            return this;
        }
        ...
        {
            Duty = $"{name}<|>{description}<|>{amount.ToString(CultureInfo.InvariantCulture)}";
            return this;
        }
        ...
        {
            Duty = amount.ToString(CultureInfo.InvariantCulture);
            return this;
        }
        ...
        {
            Tax = $"{name}<|>{description}<|>{amount.ToString(CultureInfo.InvariantCulture)}";
            return this;
        }
        ...
        {
            Tax = amount.ToString(CultureInfo.InvariantCulture);
            return this;
        }
        ...
        /// </summary>
        public IGatewayRequest AddShipping(string ID, string email, string first, string last, string address, string city, string state,
            string zip)
        {
        ...
        {
            var sFld = ApiFields.LineItem;
            var lineFormat =
                $"{itemID}<|>{name}<|>{description}<|>{quantity}<|>{price.ToString(CultureInfo.InvariantCulture)}<|>{taxable}";
        ...
        {
            Queue(ApiFields.DelimitData, "TRUE");
            Queue(ApiFields.DelimitCharacter, "|");
            Queue(ApiFields.RelayResponse, "FALSE");
            Queue(ApiFields.Method, "CC");
            Queue(ApiFields.Version, "3.1");
        }
